    ...
    for (var i = 0; i < 6; i++) {
      ...
      for (var d = 0; d < 7; d++) {
        ...
        if (day < startDay) {
          ...
          const active = !Object.keys(this.props.days).every(l => this.props.days[l].every(e => !e.startOf('day').isSame(this.props.month.clone().set('date', day - startDay + 1).startOf('day'))));
...
class AppointmentTable extends Component {
  render() {
    ...
    const body = [];
    const numLocations = Object.keys(slots).length;

    const d = {};
    Object.keys(slots).forEach(v => {
      Object.keys(slots[v]).forEach(t => {
        if(d[t]) {
          d[t].push(v);
        } else {
          d[t] = [v];
      ...
      <table className='table'>
        <thead>
          <tr className='text-center'>
            {Object.keys(slots).map(v => (
              <th scope='col' key={v}>{v}</th>
            ))}
          </tr>
        </thead>
